...
export function InsomniaDotNav({ activeId, onNavigate }: InsomniaDotNavProps) {
  ...
  return (
    <nav
      ...
      className="pointer-events-auto fixed right-8 top-1/2 z-50 hidden -translate-y-1/2 min-[1280px]:flex min-[1280px]:flex-col min-[1280px]:items-end"
    >
      <div
        ref={wrapRef}
        ...
        {INSOMNIA_DOT_NAV_SECTIONS.map((section, idx) => {
          ...
          return (
            ...
            >
              <span
                className={cn(
                  "pointer-events-none absolute right-[calc(0.5rem+8px)] mr-3 whitespace-nowrap font-heading text-[11px] uppercase tracking-[0.12em] transition-[opacity,transform,color] duration-200 ease-out",
                  showLabel
                    ? "mr-4 translate-x-0 opacity-100 text-shadow-sm/100"
                    : "mr-4 translate-x-1 opacity-0",
                )}
              ...
              >
                <span
                  ...
                  style={{
                    width: W0,
                    height: H0,
                    backgroundColor: isHover ? ACCENT : INACTIVE_DOT,
                    boxShadow: isHover
